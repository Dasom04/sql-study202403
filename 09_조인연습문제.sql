...
SELECT
    ...
    d.department_name
FROM employees e
LEFT JOIN departments d  -- INNER JOIN 하면 데이터가 안나올 수 있음
ON e.department_id = d.department_id
...
ORDER BY 사원번호;  -- 기본이 오름차순이라 ASC생략 가능
...
JOIN jobs j
ON e.job_id = j.job_id
WHERE j.job_title = 'Stock Manager' --IN ('Stock Manager', 'Stock Clerk') 도 가능
...
FROM employees e1
JOIN employees e2
ON e1.manager_id = e2.employee_id; -- 매니저도 사원이라서 매칭을 시킬 수 있다.
