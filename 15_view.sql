...
-- 뷰의 컬럼명은 함수 호출문, 연산식 등 간은 가상 표현식이면 안됩니다.
SELECT 
    employee_id,
...
CREATE VIEW view_emp AS (
    SELECT 
        employee_id,
        first_name || ' ' || last_name AS full_name, -- 별칭을 지정해줘야 한다. 문자 연결식만 있으면 안됨.
        job_id,
        salary
    FROM employees
    WHERE department_id = 60
...
-- 동일 이름으로 해당 구문을 사용하면 데이터가 변경되면서 새롭게 생섭됩니다.
...
-- 두번째 컬럼인 'full_name'은 가상열(vir) 이기 때문에 INSERT 안됩니다.
...
-- 원본 테이블 컬럼 중 NOT NULL 컬럼이 존재하고, 해당 컬럼을 view로 지고할 수 없다면 INSERT가 불가합니다.
...
-- 여러개가 복합된곳은 반영이 안되지면 일반 테이블은 반영됨
SELECT *FROM view_emp;
...
-- 뷰를 생설할때 사용한 조건 컬럼은 뷰를 통해서 변경할 수 없게 해주는 키워드
...
WHERE employee_id = 107;
-- 뷰를 생설할때 걸어줬던 조건에 새언된 갑ㅅ을 변경하지 못하게 할때 쓴다
...
-- 뷰는 가상의 테이블이다 그래서 용량을 먹지 않는다.
-- 외부로 노출하고 싶지 않은걸 뷰로 사용한다.
