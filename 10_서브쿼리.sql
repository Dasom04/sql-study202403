...
SELECT * FROM employees   -- 메인쿼리
WHERE job_id = (SELECT job_id FROM employees
                WHERE employee_id = 103);  -- 서브쿼리
                

-- 다음문장은 서브쿼리가 리턴하는 행이 여러 개라서 단일행 연산자를 사용할 수 없습니다.
...
-- SOME도 동일 결과가 나온다. ANY를 더 많이 사용한다.
SELECT * FROM employees
WHERE salary > ANY (SELECT salary FROM employees
                    WHERE first_name = 'David');  -- David의 가장 낮은 값보다 크면 된다. (4800보다 크면 된다.)
...
WHERE salary > ALL (SELECT salary FROM employees
                    WHERE first_name = 'David'); -- David의 최대급여 값보다 더 커야한다. (9500보다 커야 된다.)


-- EXISTS: 서브쿼리가 하나 이상의 행을 반환하면 참으로 간주.
-- job_histiry에 존재하는 직원이 employees에도 존재한다면 조회대상에 포함.
...
WHERE EXISTS (SELECT 1 FROM departments d
              WHERE e.department_id = d.department_id); -- e.department_id = d.department_id가 같다면 1을 띄우겠다.
-- 존재하는게 확인되면 employees의 전체 컬럼을 꺼내고싶다.
/*
가로 안을 실행하고 싶다면 employees e를 넣어줘야함.
from에 어느 테이블에서 가져오는지를 써줘야함
ex)(SELECT 1 FROM job_history jh , employees e     
              WHERE e.employee_id = jh.employee_id);
*/
