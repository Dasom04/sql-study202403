...
-- GROUP BY절을 사용할때 GROUP절에 묶이지 않은 컬럼은 조회할 수 없습니다.
SELECT 
    job_id,
    department_id,
    AVG(salary)
...
-- GROUP BY를 통해 그룹화 할때 조건을 걸 경우 HAVING을 사용.
...
FROM employees
GROUP BY department_id
HAVING AVG(salary) >= 10000;  -- GROUP BY의 조건절
...
-- 부서 아이디가 50 이상인 것들을 그룹 굽여 평균이 5000 이상만 조회
SELECT
    department_id,
    AVG(salary) AS 평균
FROM employees
