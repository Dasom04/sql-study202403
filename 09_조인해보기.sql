...
WHERE e.department_id = d.department_id; -- JOIN의 조건을 써준다. (오라클의 조인 문법)

SELECT
    e.first_name,  -- 각 컬럼이 각자 가지고있는 컬럼이면 앞에 소속 테이블 이름 안붙여됨. 하지만 붙여주는게 더 좋음
    d.department_name,
    d.department_id
FROM employees e
...
WHERE e.department_id = d.department_id(+)
AND d.location_id = loc.location_id(+);  -- d.location_id에 loc.location_id가 갖다 붙여진다.
/*
