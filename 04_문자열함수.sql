...
-- lower (소문자 일괄변환), initcap(앞글자만 대문자), upper (대문자 일괄변환)
...
--substr(자를 문자열, 시작인덱스, 길이) ->  길이 안정하면 끝까지 나옴.
--substr(자를 문자열, 시작인덱스) -> 문자열 끝까지. num번 부터 num까지 주세요 라는 뜻.
-- 인덱스 1부터 시작
-- concat(문자연결)
