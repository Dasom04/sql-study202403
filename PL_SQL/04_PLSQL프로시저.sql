...
CREATE PROCEDURE guguproc
    (p_dan In NUMBER) -- 매개변수  / IN: 바깥 족에서 안쪽으로 전달된다 라는 뜻
...
BEGIN
    ...
    IF v_cnt = 0 THEN -- 조회 결과가 없었다면 INNSERT
        INSERT INTO jobs
        VALUES(p_job_id, p_job_title, p_min_sal, p_max_sal);
...
BEGIN
    ...
    IF v_cnt = 0 THEN -- 조회 결과가 없엇다면 INNSERT
        INSERT INTO jobs
        VALUES(p_job_id, p_job_title, p_min_sal, p_max_sal);
    ELSE -- 조회 결과가 있다면 UPDATE
        UPDATE jobs
...
-- OUT을 이용해서 보낸 값은 바깥 익명 블록에서 실핼해야 합니다.
CREATE OR REPLACE PROCEDURE my_new_job_proc
    (p_job_id IN jobs.job_id%TYPE,
     p_job_title IN jobs.job_title%TYPE,
     p_min_sal IN jobs.min_salary%TYPE := 0,
...
IS
    v_cnt NUMBER := 0;
    v_result VARCHAR2(100) := '존재하지 않는 값이라서 INNSERT 처리 되었습니다.';
BEGIN
    ...
    -- OUT 매개변수에 값을 할당해 놓으면 프로시저 정료 후 호출부로 OUT 변수의 값이 전달됩니다.
    p_result := v_result;
    COMMIT;
END;
...
BEGIN
    ...
    my_new_job_proc('CEO1', 'test_ced1', 30000, 80000, msg);
    dbms_output.put_line(msg);
END;
...
BEGIN
    ...
    dbms_output.put_line('p_var3: ' || p_var3);  -- IN OUT: IN의 정실을 가지고 있구나~

    -- p_var1 := '결과1'; -> IN변수는 값 할당 자체 불가능.
    p_var2 := '결과2'; 
    p_var3 := '결과3';
...
-- RETURN: 값의 반환X, 프로시저를 강제 종료할 때 사용.
CREATE OR REPLACE PROCEDURE my_new_job_proc
    (p_job_id IN jobs.job_id%TYPE,
