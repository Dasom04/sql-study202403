...
-- 서식문자와 함께 사용하고싶은 문자를 ""로 감싸서 전달합니다.
SELECT 
    TO_CHAR(sysdate, 'YYYY"년" MM"월" DD"일"') 
FROM dual;

-- 숫자를 문자로 TO_CHAR(값, 형식) (99,999는 자릿수를 위해 쓰는것)
...
SELECT '$3,300' + 2000 FROM dual; -- 기호나 다른게 섞여있으면 자동 형변환 안해줌.
...
-- NULL 형태를 반환하는 함수 NVL2(컬럼, null이 아닐 경우의 값, null일 경우의 값)
SELECT 
    first_name,
    salary,
    NVL2(
        commission_pct, -- 기준이 되는 컬럼  null 이거나 아니거나 하는 값이 있어야 함.
        salary + (salary * commission_pct), -- null이 없다면 이러한 값
        salary -- null이 있다면 이러한 값
    ) AS real_salary
FROM employees
